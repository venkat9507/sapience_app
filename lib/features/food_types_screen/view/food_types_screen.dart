...
import 'package:nimu_tv/features/food_days_screen/bloc/food_days_bloc.dart';
...
import '../../../main.dart';
import '../../food_days_screen/database_items/items_data.dart';
...
import '../bloc/food_types_bloc.dart';
import '../database_items/items_data.dart';

class FoodTypesScreenView extends StatefulWidget {
  final String? title;

  const FoodTypesScreenView(
      {Key? key, this.title, })
      : super(key: key);

  @override
  State<FoodTypesScreenView> createState() => _FoodTypesScreenViewState();
}

class _FoodTypesScreenViewState extends State<FoodTypesScreenView> {
  final FoodTypesBloc foodTypesBloc = FoodTypesBloc();

  @override
  void initState() {
    // TODO: implement initState
    print('food days screen length check  ${foodDaysModel?.data.length} ');
    ...
    foodTypesBloc.add(FoodTypesInitialEvent(
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<FoodTypesBloc, FoodTypesState>(
      bloc: foodTypesBloc,
      listenWhen: (previous, current) => current is FoodTypesActionState,
      buildWhen: (previous, current) => current is!FoodTypesActionState,
      listener: (context, state) {
        if (state is FoodTypesActionButtonState) {
          Navigator.pushAndRemoveUntil(context,
              PageTransition(
                  type: PageTransitionType.bottomToTop,
                  duration: const Duration(seconds: 1),
                  child:  FoodDaysScreenView(title: state.title,)),(Route<dynamic> route) => false);
        ...
        else if (state is FoodTypesWatchOnlineActionButtonState) {
          // Navigator.push(
          //   context,
          //   PageTransition(
          //       type: PageTransitionType.scale,
          ...
          // Navigator.push(context, MaterialPageRoute(builder: (context) => const AddProductScreen()));
         ...
         else if (state is FoodTypesBackActionButtonState) {
           if(storingFoodCategoryInsideScreenNo ==0) {
             Navigator.pushAndRemoveUntil(
                 ...
                 PageTransition(
                     ...
                     duration: const Duration(milliseconds: 100),
                     child: LkgScreenView(
                       sectionID: foodLkgCategoryModel!.data[0].sectionId,
                     )),
                     (Route<dynamic> route) => true);
           }
           else if (storingFoodCategoryInsideScreenNo ==1){
           Navigator.pushAndRemoveUntil(
           context,
           PageTransition(
           type: PageTransitionType.scale,
           ...
           duration: const Duration(milliseconds: 100),
        child: UkgScreenView(
        sectionID: foodUkgCategoryModel!.data[0].sectionId,
        )),
        (Route<dynamic> route) => true);
      ...
      builder: (context, state) {
        switch (state.runtimeType) {
          case FoodTypesInitialLoadingState:
            return circularProg!;

          case FoodTypesInitialLoadedSuccessState:
            final successState = state as FoodTypesInitialLoadedSuccessState;
            ...
            return WillPopScope(
              onWillPop: () async {
                ...
                foodTypesBloc
                    .add(FoodTypesBackNavigateButtonEvent());
                return await true;
              },
              child: Scaffold(
                        ...
                        SingleChildScrollView(
                          ...
                          child: Column(
                            children: [
                              ...
                              HomePage(
                                foodTypesBloc: foodTypesBloc,
                                foodTypeList: successState.foodTypesModel!.data,
                              ),
                              // Positioned(
                              //   top: 20,
                              //   left: 30,
          ...
          case FoodTypesInitialErrorState:
            final errorState = state as FoodTypesInitialErrorState;
...
class HomePage extends StatefulWidget {
  ...
  const HomePage({Key? key, this.foodTypeList, this.foodTypesBloc, })
      : super(key: key);

  final List<FoodTypesList>? foodTypeList;

  final FoodTypesBloc? foodTypesBloc;
                        ...
                        child: TextButton(
                          onPressed: () {
                            if (PlatformType.isTv != true) {
                              foodTypesModelIndex = i;
                              widget.foodTypesBloc!.add(FoodTypesNavigateButtonEvent(
                                foodTypeID: widget.foodTypeList![i].id,
                                title: widget.foodTypeList![i].name,
                              ));

                              foodDaysString.value = widget.foodTypeList![i].name;
                            }
                          },
                          ...
                          child: Container(
                            ...
                            child: Column(
                              children: [
                                Padding(
                                  padding:  EdgeInsets.only(left: 10 * fem,top: 40 * fem),
                                  child: CachedNetworkImage(
                                    ...
                                    imageUrl: widget.foodTypeList![i].image,
                                    // fit: BoxFit.contain,
